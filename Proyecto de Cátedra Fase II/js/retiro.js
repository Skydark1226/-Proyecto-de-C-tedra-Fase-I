(function() {
      ...
      var constraints = {
        retiro: {
			...
			numericality: {
				onlyInteger: false,
				greaterThan: 0,
				notGreaterThan: "no puede ser 0",
				lessThanOrEqualTo: parseFloat(localStorage.getItem("saldo_inicio")),
				notLessThanOrEqualTo: "no puede ser mayor al saldo actual",
				noStrings: true,
				divisibleBy: 5,
				notDivisibleBy: "Solo multiplos de 5"
			}
        }
      };
      ...
      function showSuccess() {
        // We made it \:D/}
        swal({
		  title: "¿Desea retirar $"+parseFloat(document.getElementById("retiro").value).toFixed(2)+" ?",
		...
		.then((willDelete) => {
		  if (willDelete) {
			...
			let valor_ret = parseFloat(document.getElementById("retiro").value);
			let resultado = saldo_actual - valor_ret;
			...
			let objR = {valor_ret,fecha,hora};
			retiros.push(objR);
			localStorage.setItem('retiros', JSON.stringify(retiros));
			
			swal("¡Retiro efectuado con exito!", {
			  icon: "success",
			});
			
			swal({
			  title: "Retiro Efectuado con EXITO!!!",
			...
			.then((comprobante) => {
			  if (comprobante) {
				...
				doc.setFontSize(22);
				doc.text(20, 50, 'Comprobante de Retiro');
				doc.setFontSize(14);
				doc.text(20, 60, 'Valor Retiro: $'+parseFloat(valor_ret).toFixed(2));
